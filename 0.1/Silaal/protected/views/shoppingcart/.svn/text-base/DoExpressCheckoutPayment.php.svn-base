	...
	if(!empty($midle) || !empty($resArray)){
		...
		if($address->save()){
			...
			if($customer->save()){
				...
				$order->comment = '';
				if($order->save()){
					$counter = ProductCounter::model()->findByAttributes(array('product_id'=>$order->product_id));
					if($counter!== null){
		...
		if(isset($checkout['order_id']))
			Order::model()->updateByPk($checkout['order_id'], array('ordering_confirmed'=>1));
		else{
